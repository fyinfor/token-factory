...
func ImageHelper(c *gin.Context, info *relaycommon.RelayInfo) (tokenFactoryError *types.TokenFactoryError) {
	...
	requestBodyBytes, bodyErr := buildImageUpstreamRequestBody(c, info, adaptor, request)
	if bodyErr != nil {
		return bodyErr
	}

	if callbackURL != "" {
		return launchImageAsyncCallback(c, info, request, requestBodyBytes, callbackURL)
	}

	return executeImageRelay(c, info, request, bytes.NewReader(requestBodyBytes), false)
}

func buildImageUpstreamRequestBody(c *gin.Context, info *relaycommon.RelayInfo, adaptor channel.Adaptor, request *dto.ImageRequest) ([]byte, *types.TokenFactoryError) {
	if model_setting.GetGlobalSettings().PassThroughRequestEnabled || info.ChannelSetting.PassThroughBodyEnabled {
		...
		stripped, stripErr := stripCallbackURLFromRequestBody(data, c.Request.Header.Get("Content-Type"))
		if stripErr != nil {
...
func launchImageAsyncCallback(c *gin.Context, info *relaycommon.RelayInfo, request *dto.ImageRequest, requestBody []byte, callbackURL string) *types.TokenFactoryError {
	...
	gopool.Go(func() {
		...
		tokenFactoryError := executeImageRelay(asyncCtx, info, requestCopy, bytes.NewReader(bodyCopy), true)
		...
		if capture, ok := asyncCtx.Get("image_async_response_body"); ok {
			if raw, ok := capture.([]byte); ok && len(raw) > 0 {
				...
				if err := common.Unmarshal(raw, &imageResp); err == nil {
					...
					logger.LogWarn(asyncCtx, fmt.Sprintf("parse async image response failed: %s", err.Error()))
				}
			}
		}
...
func executeImageRelay(c *gin.Context, info *relaycommon.RelayInfo, request *dto.ImageRequest, requestBody io.Reader, asyncCapture bool) (tokenFactoryError *types.TokenFactoryError) {
	adaptor := GetAdaptor(info.ApiType)
	if adaptor == nil {
		return types.NewError(fmt.Errorf("invalid api type: %d", info.ApiType), types.ErrorCodeInvalidApiType, types.ErrOptionWithSkipRetry())
	}
	...
	if captureWriter != nil && captureWriter.captureOnly {
		...
		if asyncCapture {
			c.Set("image_async_response_body", append([]byte(nil), responseCapture.Bytes()...))
		} else {
