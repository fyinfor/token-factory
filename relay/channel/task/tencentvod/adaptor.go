...
func (a *TaskAdaptor) EstimateBilling(_ *gin.Context, _ *relaycommon.RelayInfo) map[string]float64 { return nil }
...
func (a *TaskAdaptor) BuildRequestURL(_ *relaycommon.RelayInfo) (string, error) {
	return normalizeVodEndpoint(a.baseURL) + "/", nil
}
...
func (a *TaskAdaptor) BuildRequestBody(c *gin.Context, info *relaycommon.RelayInfo) (io.Reader, error) {
	...
	if p := strings.TrimSpace(req.GetPrompt()); p != "" {
		body["Prompt"] = p
	}
	...
	if err := common.Unmarshal(originTask.Data, &env); err == nil && env.Response != nil {
		if env.Response.Error != nil && strings.TrimSpace(env.Response.Error.Message) != "" {
			ov.Error = &dto.OpenAIVideoError{
				Message: strings.TrimSpace(env.Response.Error.Message),
				Code:    firstNonEmpty(strings.TrimSpace(env.Response.Error.Code), "tencent_vod_error"),
			}
		}
		if env.Response.AigcVideoTask != nil && env.Response.AigcVideoTask.Output != nil {
			for _, fi := range env.Response.AigcVideoTask.Output.FileInfos {
...
func (a *TaskAdaptor) DoRequest(_ *gin.Context, info *relaycommon.RelayInfo, requestBody io.Reader) (*http.Response, error) {
	...
	return SignedPOSTJSON(proxy, endpoint, cred.Region, cred, "CreateAigcVideoTask", payload)
}
...
func (a *TaskAdaptor) FetchTask(baseURL, key string, body map[string]any, proxy string) (*http.Response, error) {
	...
	endpoint := normalizeVodEndpoint(baseURL)
	return SignedPOSTJSON(strings.TrimSpace(proxy), endpoint, cred.Region, cred, "DescribeTaskDetail", payload)
}
...
func (a *TaskAdaptor) ConvertToOpenAIVideo(originTask *model.Task) ([]byte, error) {
	...
	if err := common.Unmarshal(originTask.Data, &env); err == nil && env.Response != nil {
		...
		if ov.Error == nil && originTask.Status == model.TaskStatusFailure {
			failMsg := ""
			if env.Response.AigcVideoTask != nil && env.Response.AigcVideoTask.Message != nil {
				failMsg = strings.TrimSpace(*env.Response.AigcVideoTask.Message)
			}
			if failMsg == "" {
