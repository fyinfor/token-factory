...
const normalizeVideoRuleRow = (row) => ({
    ...
    noAudioPrice: Boolean(row?.has_audio) ? '' : toNumericString(row?.token_price),
    withAudioPrice: Boolean(row?.has_audio) ? toNumericString(row?.token_price) : '',
...
const normalizePerVideoRuleRow = (row) => ({
    ...
    noAudioPrice: Boolean(row?.has_audio) ? '' : toNumericString(row?.video_price),
    withAudioPrice: Boolean(row?.has_audio) ? toNumericString(row?.video_price) : '',
    audioPricingEnabled: Boolean(row?.has_audio),
    hasAudio: Boolean(row?.has_audio),
});
...
const parseVideoPricingRules = (rawRules) => {
    ...
    const videoToVideoInputPerVideo = normalizeAudioPricedRows(
        rawRules.video_to_video_input_per_item || rawRules.video_to_video_input_per_video,
        'video_price',
    );
    const videoToVideoOutputPerVideo = normalizeAudioPricedRows(
        rawRules.video_to_video_output_per_item || rawRules.video_to_video_output_per_video,
        'video_price',
    );
    ...
    return {
        ...
        priceUnit: String(rawRules.price_unit || '').trim().toUpperCase(),
    };
};
...
export const buildPreviewRows = (model, t, visibleCategories = null) => {
    ...
    const inputPrice = toNumberOrNull(model.inputPrice);
    const tieredBillingMode = model.billingMode === 'tiered';
    const videoPerToken = model.videoBillingMode === 'per-second';
    const videoPerVideo = model.videoBillingMode === 'per-item';
...
export function useModelPricingEditorState({
  ...
  const handleVideoPriceUnitChange = (nextUnit) => {
    if (!selectedModel || !nextUnit || nextUnit === selectedModel.videoPriceUnit) {
      return;
    }
  ...
  const updateVideoRuleRow = (section, index, key, value) => {
    if (!selectedModel) return;
    const numericKeys = new Set(['tokenPrice', 'pixelCompression', 'videoPrice']);
    ...
    const addVideoRuleRow = (section) => {
        ...
        ? { resolution: '', videoPrice: '', noAudioPrice: '', withAudioPrice: '', audioPricingEnabled: false }
                : {
                    resolution: '',
                    tokenPrice: '',
            noAudioPrice: '',
