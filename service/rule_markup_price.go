...
// EffectiveVideoPerSecondUSDForDimensions 按请求像素匹配渠道档位后，用同档全局价计算有效单价（与定价卡片一致）。
func EffectiveVideoPerSecondUSDForDimensions(
	...
	match, okMatch := matchPerSecondPriceDetail(channelRules, mode, width, height, hasAudio)
