...
type ChannelOtherSettings struct {
	...
	// SeedanceFetchAPI 火山方舟-Seedance 2.0 上游任务查询路径（双接口可配置）。
	// 空 / "contents_generations" => GET {base}/api/v3/contents/generations/tasks/{id}
	// "video_generations" => GET {base}/v1/video/generations/{id}（旧 OpenAI 兼容查询）
	SeedanceFetchAPI string `json:"seedance_fetch_api,omitempty"`
}

// ResolveSeedanceFetchAPI 优先使用任务提交时快照，缺省再读渠道当前配置。
func ResolveSeedanceFetchAPI(taskSnapshot, channelSetting string) string {
	if s := strings.TrimSpace(taskSnapshot); s != "" {
		return s
	}
	return strings.TrimSpace(channelSetting)
}
