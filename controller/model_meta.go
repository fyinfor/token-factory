...
func SearchModelsMeta(c *gin.Context) {
	...
	if c.GetInt("role") >= common.RoleAdminUser {
		modelsMeta, total, err = model.SearchModels(keyword, vendor, routeSlug, tag, pageInfo.GetStartIdx(), pageInfo.GetPageSize())
	} else {
		ownerUserID := c.GetInt("id")
		modelsMeta, total, err = model.SearchSupplierModels(&ownerUserID, keyword, vendor, routeSlug, tag, pageInfo.GetStartIdx(), pageInfo.GetPageSize())
	}
	if err != nil {
		common.ApiError(c, err)
		return
